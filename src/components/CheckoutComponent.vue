  ...
  <div class="mb-10">
    ...
    <v-card class="mx-10 mt-5">
      ...
      <v-text-field
        ...
        @input="cekVoucher"
      ></v-text-field>
      <v-card-title v-show="show_voucher" class="mx-10 pa-0" style="color: green">yeay ada diskon {{this.persen_diskon*100}}% !!!</v-card-title>
      <v-card-title class="mx-10 pa-0 py-5">
        Total harga: Rp{{ ($store.getters.getTotalHarga - ($store.getters.getTotalHarga*this.persen_diskon)).toLocaleString("id-ID") }}
      </v-card-title>
      <v-card-actions class="pb-5">
        <v-btn block x-large class="btn-css">order now</v-btn>
      </v-card-actions>
    </v-card>
  </div>
</template>
